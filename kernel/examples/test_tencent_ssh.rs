...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let private_key_path =
        PathBuf::from(std::env::var("HOME").unwrap_or_default() + "/.ssh/tencent.pem");
    ...
    match deployer.connect_with_key(host, port, username, &private_key_path, None) {
        Ok(_) => {
            ...
            match deployer
                .execute_command("ls -la /opt 2>/dev/null | head -5 || echo '需要创建/opt/aurelia'")
            {
                Ok(output) => {
                    println!("\n/opt目录状态:");
                    println!("{}", output);
                                ...
                                match deployer.check_kernel_status() {
                                    Ok(running) => {
                                        if running {
                                            ...
                                            // 显示进程信息
                                            if let Ok(ps_output) = deployer.execute_command(
                                                "ps aux | grep kernel | grep -v grep",
                                            ) {
                                                println!("\n进程信息:");
                                                println!("{}", ps_output);
                                            }
