...
impl ServerConfig {
    ...
    pub fn get_enabled_servers(&self) -> Vec<&TargetServer> {
        self.target_servers.iter().filter(|s| s.enabled).collect()
    }
    ...
    pub fn get_server_by_id(&self, id: &str) -> Option<&TargetServer> {
        self.target_servers.iter().find(|s| s.id == id)
    }
    ...
    pub fn update_server(
        &mut self,
        id: &str,
        updater: impl FnOnce(&mut TargetServer),
    ) -> Result<()> {
        let server = self
            .target_servers
            .iter_mut()
