...
impl TaskScheduler {
    ...
    pub async fn register_executor(&self, task_type: TaskType, executor: Box<dyn TaskExecutor>) {
        self.task_executors.write().await.insert(task_type, executor);
    }
    ...
    async fn validate_dependencies(&self, task: &Task) -> bool {
        ...
        for dep_id in &task.dependencies {
            let dep_completed = completed.iter()
    ...
    async fn execute_task(&self, mut task: Task) {
        ...
        self.running_tasks.write().await.insert(task_id.clone(), task.clone());
    ...
    pub async fn get_status(&self) -> SchedulerStatus {
        SchedulerStatus {
            ...
            next_task_time: self.task_queue.read().await
                .peek()
