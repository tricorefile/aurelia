use crate::config::{TestConfig, ServerRole};
...
impl TestRunner {
    ...
    async fn pre_deployment_checks(&self) -> Result<()> {
        ...
        for server in &self.config.test_environments {
            ...
            match client.test_connection() {
                ...
                Ok(false) => return Err(anyhow::anyhow!(
                    "Connection test to {} failed", server.name
                )),
                Err(e) => return Err(anyhow::anyhow!(
                    "Cannot connect to {}: {}", server.name, e
                )),
            }
        }
    ...
    async fn deploy_all_agents(&self) -> Result<()> {
        ...
        for server in &self.config.test_environments {
            ...
            client.deploy_agent(&self.binary_path)
    ...
    async fn test_self_replication(&self) -> Result<()> {
        ...
        let primary = self.config.get_primary_server()
        ...
        match replica_monitor.verify_replica_deployment() {
            ...
            Ok(false) => return Err(anyhow::anyhow!("Self-replication failed - replica not found")),
    ...
    async fn continuous_monitoring(&self) -> Result<()> {
        ...
        info!("Starting continuous monitoring for {} minutes...", duration_minutes);
        ...
        while start_time.elapsed() < total_duration {
            ...
            info!("Monitoring... ({} minutes remaining)", 
