use crate::config::{TestConfig, ServerConfig, ServerRole};
...
impl ValidationSuite {
    ...
    pub async fn run_full_validation(&mut self) -> Result<ValidationSummary> {
        ...
        let replicas: Vec<_> = self.config.get_replica_servers().into_iter().cloned().collect();
        if let Some(primary_server) = primary {
            if !replicas.is_empty() {
                self.validate_self_replication(&primary_server, &replicas[0]).await?;
            }
        }
        ...
        info!("Validation suite completed: {} passed, {} failed", 
        ...
        for server in &self.config.test_environments {
            ...
            match monitor.check_agent_health() {
                Ok(health) => {
                    ...
                    result.details.insert("is_running".to_string(), 
                                        serde_json::json!(health.is_running));
                    result.details.insert("cpu_usage".to_string(), 
                                        serde_json::json!(health.cpu_usage));
                    result.details.insert("memory_mb".to_string(), 
                                        serde_json::json!(health.memory_mb));
        ...
        for server in &self.config.test_environments {
            ...
            match monitor.get_resource_metrics() {
                Ok(metrics) => {
                    ...
                    result.details.insert("cpu_percent".to_string(), 
                                        serde_json::json!(metrics.cpu_percent));
                    result.details.insert("cpu_limit".to_string(), 
                                        serde_json::json!(limits.max_cpu_percent));
                    result.details.insert("memory_mb".to_string(), 
                                        serde_json::json!(metrics.memory_mb));
                    result.details.insert("memory_limit_mb".to_string(), 
                                        serde_json::json!(limits.max_memory_mb));
                    result.details.insert("disk_gb".to_string(), 
                                        serde_json::json!(metrics.disk_usage_gb));
                    result.details.insert("disk_limit_gb".to_string(), 
        ...
        for server in &self.config.test_environments {
            ...
            match monitor.check_log_activity() {
                Ok((active, logs)) => {
                    ...
                    result.details.insert("has_activity".to_string(), 
                                        serde_json::json!(active));
                    result.details.insert("log_count".to_string(), 
                                        serde_json::json!(logs.len()));
                    
                    if !logs.is_empty() {
                        result.details.insert("recent_logs".to_string(), 
        ...
        for server in &self.config.test_environments {
            ...
            match monitor.check_network_connections() {
                Ok(count) => {
                    ...
                    result.details.insert("websocket_connections".to_string(), 
    ...
    async fn validate_self_replication(&mut self, primary: &ServerConfig, replica: &ServerConfig) -> Result<()> {
        info!("Validating self-replication from {} to {}", 
        ...
        match replica_monitor.verify_replica_deployment() {
            Ok(exists) => {
                ...
                result.details.insert("replica_deployed".to_string(), 
                                    serde_json::json!(exists));
                
                if exists {
                    // Check if replica is running
                    if let Ok(health) = replica_monitor.check_agent_health() {
                        result.details.insert("replica_running".to_string(), 
                                            serde_json::json!(health.is_running));
                    }
                }
            }
            Err(e) => {
                result.errors.push(format!("Replication check failed: {}", e));
            }
        }
    ...
    fn generate_summary(&self, start_time: DateTime<Utc>, end_time: DateTime<Utc>) -> ValidationSummary {
